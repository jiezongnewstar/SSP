...
2. 使用$[]
3. 使用let命令
4. 使用expr外部程式(脚本方式  建议用此方法)

- 重定向

在linux中，输出的地方分为两个，一个是显示器，另一个是文件。我们可以通过显示器输入，或者输出，也可以通过文件输出。这里说到的输出的地方我们称作 __描述符__，并通过数字的方式来定义：
...
___注意：shell 函数执行是由上到下，所以先要定义好函数之后再进行函数调用___

> 这里补充一下， shell 函数的参数 需要在调用的时候 用空格 + 参数的形式传入，参数取值是通过 $1 等数字下标来取，根据参数数量来决定的[传送门-func.sh]

> 参数返回值需要通过将函数执行结果赋给变量，然后echo 变量[传送门-func.sh]
